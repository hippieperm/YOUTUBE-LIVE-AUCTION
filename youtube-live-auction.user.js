  ...
  <WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel">
   ...
   <ProtectScenarios>False</ProtectScenarios>
    ...
    function openUnifiedPriceModal() {
        ...
        const title = createElement('div', { text:'💰 가격 입력', style:'font-size:17px !important; font-weight:900 !important; color:#fde047 !important;' });
        ...
        [['👑 최고가','최고가'],['⬆️ 이상','이상'],['📋 일반','일반']].forEach(([label,value]) => { const tab=createElement('button',{type:'button',text:label,style:'height:42px !important; border:1px solid rgba(148,163,184,.22) !important; border-radius:11px !important; background:rgba(255,255,255,.05) !important; color:#cbd5e1 !important; font-size:12px !important; font-weight:900 !important; cursor:pointer !important;'}); tab.dataset.mode=value; tabs.appendChild(tab); });
        const input = createElement('input', { type:'text', inputmode:'decimal', placeholder:'금액 입력', style:'flex:1 !important; width:100% !important; height:42px !important; box-sizing:border-box !important; padding:0 12px !important; border:1px solid rgba(148,163,184,.3) !important; border-radius:11px !important; background:rgba(8,15,30,.85) !important; color:#fff !important; -webkit-text-fill-color:#fff !important; font-size:17px !important; font-weight:800 !important; outline:none !important;' });
        const inputWrap = createElement('div', { style:'display:flex !important; align-items:center !important; gap:8px !important; margin-top:14px !important;' }); const unit=createElement('span',{text:'만원',style:'font-size:14px !important; font-weight:800 !important; color:#cbd5e1 !important;'}); inputWrap.appendChild(input); inputWrap.appendChild(unit); modal.appendChild(tabs); modal.appendChild(inputWrap);
        ...
        const makeStepButton = (label, value) => { const b=createElement('button',{type:'button',text:label,style:'height:38px !important; border:1px solid rgba(250,204,21,.35) !important; border-radius:10px !important; background:rgba(250,204,21,.1) !important; color:#fde68a !important; font-size:12px !important; font-weight:900 !important; cursor:pointer !important;'}); b.addEventListener('click',()=>{const current=parseFloat(input.value.replace(/,/g,''))||0;input.value=String(Math.max(0,current+value));input.focus();}); return b; };
        const oneColumn=createElement('div',{style:'display:flex !important; flex-direction:column !important; gap:6px !important;'}); oneColumn.appendChild(makeStepButton('-1만',-1)); oneColumn.appendChild(makeStepButton('+1만',1)); quick.appendChild(oneColumn); quick.appendChild(makeStepButton('+5만',5)); quick.appendChild(makeStepButton('+10만',10)); modal.appendChild(quick);
        const submit=createElement('button',{type:'button',text:'📤 입력 완료',style:'width:100% !important; height:46px !important; margin-top:16px !important; border:1px solid rgba(250,204,21,.55) !important; border-radius:13px !important; background:linear-gradient(135deg,#a16207,#ca8a04) !important; color:#fff !important; font-size:14px !important; font-weight:900 !important; cursor:pointer !important;'}); modal.appendChild(submit);
        const updateMode=next=>{mode=next;Array.from(tabs.children).forEach(tab=>{const active=tab.dataset.mode===mode;tab.style.setProperty('background',active?'rgba(250,204,21,.2)':'rgba(255,255,255,.05)','important');tab.style.setProperty('border-color',active?'rgba(250,204,21,.7)':'rgba(148,163,184,.22)','important');tab.style.setProperty('color',active?'#fde047':'#cbd5e1','important');});const max=mode==='최고가';inputWrap.style.display=max?'none':'flex';quick.style.display=max?'none':'grid';submit.textContent=max?'👑 최고가 입력':'📤 입력 완료';};
        ...
        Array.from(tabs.children).forEach(tab=>tab.addEventListener('click',()=>updateMode(tab.dataset.mode))); input.addEventListener('input',()=>{input.value=sanitizeDecimalInput(input.value);}); input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();submitPrice();}}); submit.addEventListener('click',submitPrice); backdrop.addEventListener('click',e=>{if(e.target===backdrop)removeCustomModals();}); modal.addEventListener('wheel',e=>{e.preventDefault();e.stopPropagation();},{passive:false}); mountTarget.appendChild(backdrop);mountTarget.appendChild(modal);updateMode('일반');setTimeout(()=>input.focus(),40);
    }

    function openPriceChoiceModal() {
        openUnifiedPriceModal();
        return;
