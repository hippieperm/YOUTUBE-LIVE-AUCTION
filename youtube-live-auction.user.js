...
(function () {
    ...
    function parseBidPrice(text) {
        ...
        const manChonMatch =
            clean.match(
                /(?:^|[^\d가-힣a-zA-Z])(\d+(?:\.\d+)?)\s*만\s*(\d+(?:\.\d+)?)\s*(천|000|원)?(?![가-힣a-zA-Z0-9])/i
            );
        ...
        // 2. "15만", "15만원", "15.5만", "0.5만", ".5만" (단, '만듬', '만족' 등 일반 단어 제외)
        const manMatch =
            clean.match(
                /(?:^|[^\d가-힣a-zA-Z])(\d+(?:\.\d+)?)\s*만(?:\s*원)?(?![가-힣a-zA-Z0-9])/
            );
        ...
        // 3. "5천", "5천원", "3천", "3천원" (단, '3천으로', '3천개' 등 조사/단위 접미사 제외)
        const chonMatch =
            clean.match(
                /(?:^|[^\d가-힣a-zA-Z])(\d+(?:\.\d+)?)\s*천(?:\s*원)?(?![가-힣a-zA-Z0-9])/
            );
        ...
        const wonMatch =
            clean.match(
                /(?:^|[^\d가-힣a-zA-Z])(\d{4,9})\s*원(?![가-힣a-zA-Z0-9])/
            );
        ...
        const fourDigitChonMatch =
            clean.match(
                /(?:^|[^\d가-힣a-zA-Z])([1-9]000)(?![가-힣a-zA-Z0-9])/
            );
        ...
        // 8. 일반 숫자 (예: "15", "20", "15.5", "3.5", "0.5", "2", "35")
        const numMatch =
            clean.match(
                /(?:^|[^\d가-힣a-zA-Z])(\d+(?:\.\d+)?)(?![가-힣a-zA-Z0-9])/
            );
